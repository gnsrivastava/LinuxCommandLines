...
1. **List [```ls``` ]**<br>
   ...
   ls
   ```
   There various useful options that users of Linux employ to make this results of the ```ls``` command more intuitive/informative
   ```bash
   ls -ltrh
   ```
   ...
   The above ```ls -ltrh``` can be useful when you want to see which is the **most recent** file that you as a user have created and what are the details of that file. 
